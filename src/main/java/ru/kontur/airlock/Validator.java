...
public class Validator {
    ...
    public boolean validate(String eventRoutingKey) {
        return validateForbiddenCharacters(eventRoutingKey) && validateApiKeyAccess(
                eventRoutingKey);
    }
    ...
    private boolean matches(String eventRoutingKey, String pattern) {
        if (pattern.endsWith("*")) {
            return eventRoutingKey.toLowerCase()
                    .startsWith(pattern.substring(0, pattern.length() - 1).toLowerCase());
        } else {
