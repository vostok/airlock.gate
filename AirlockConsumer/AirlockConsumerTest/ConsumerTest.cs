...
namespace AirlockConsumerTest
{
    ...
    public class AirlockLogEventConsumerStub : AirlockConsumer<LogEventData>
    {
        public AirlockLogEventConsumerStub(IMessageProcessor<LogEventData> messageProcessor, ILog log) :
            base(AirlockEventTypes.Logging, 1000, new LogEventDataAirlockDeserializer(), messageProcessor,
                log)
        {
        }
    }
    ...
    public class ConsumerTest
    {
        ...
        public void SendAndReceive()
        {
            ...
            {
                EventGroups = new List<EventGroup>() { new EventGroup() { EventType = AirlockEventTypes.Logging, EventRecords = new List<EventRecord>()
                {
                    new EventRecord()
                    {
